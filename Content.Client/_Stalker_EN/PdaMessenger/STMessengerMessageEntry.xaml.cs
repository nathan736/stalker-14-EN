...
public sealed partial class STMessengerMessageEntry : BoxContainer
{
    ...
    private static readonly Regex OfferRefPattern = new(@"\[MB#(\d+)\]", RegexOptions.Compiled);
    ...
    /// <c>[MB#N]</c> patterns with clickable offer link markup.
    ...
    private static FormattedMessage BuildMessageContent(string content)
    {
        ...
        foreach (Match match in OfferRefPattern.Matches(content))
        {
            ...
            // Add offer link markup — controlled markup, not user input
            var id = match.Groups[1].Value;
            msg.AddMarkupPermissive($"[offerlink={id}][/offerlink]");
